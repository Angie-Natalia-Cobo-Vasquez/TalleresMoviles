...
class UniversidadesFbListView extends StatelessWidget {
  const UniversidadesFbListView({super.key});
...
class _UniversidadesCard extends StatelessWidget {
  final UniversidadesFb universidades;
  final int index;
  final bool isGridView;

  const _UniversidadesCard({
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(12),
        onTap: () => context.push('/universidadesfb/edit/${universidades.id}'),
        child: Padding(
  ...
  Future<void> _showDeleteDialog(BuildContext context) async {
    ...
    if (confirmar == true && context.mounted) {
      try {
        await UniversidadesService.deleteUniversidades(universidades.id);
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Universidad "${universidades.nombre}" eliminada'),
